...
├── pydhcp.env          # Shared config template (network, DNS, ACL paths)
...
| Shared config (network, DNS, ACL paths) used by `pyinstall.sh` and `tools/pyleases.sh` | Configuración compartida (red, DNS, rutas ACL) usada por `pyinstall.sh` y `tools/pyleases.sh` | `/etc/pydhcp/pydhcp.env` |
...
> **Config source**: `pyinstall.sh` writes `/etc/pydhcp/pydhcp.env` during installation (DHCP server IP, netmask, block-pool range, DNS servers, ACL paths). `pyleases.sh` reuses that same file and only launches its own interactive setup if `/etc/pydhcp/pydhcp.env` is missing (e.g. an installation from before this file existed). Delete it to force the setup to run again.
>
> **Origen de la configuración**: `pyinstall.sh` escribe `/etc/pydhcp/pydhcp.env` durante la instalación (IP del servidor DHCP, máscara, rango del pool de bloqueo, servidores DNS, rutas ACL). `pyleases.sh` reutiliza ese mismo archivo y solo ejecuta su propio setup interactivo si `/etc/pydhcp/pydhcp.env` no existe (p.ej. una instalación anterior a que este archivo existiera). Elimínelo para forzar que el setup se ejecute de nuevo.
