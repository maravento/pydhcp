  ...
  <tr>
    <td style="width: 50%; vertical-align: top;">
      ...
      <ul>
        ...
        <li><code>option domain-name</code> (option 15, the DNS search domain a client appends to short names). Validating it would mean resolving a name at config load, which this daemon deliberately never does, and a search domain often has no record of its own, so a resolution test would reject valid ones</li>
    ...
    <td style="width: 50%; vertical-align: top;">
      ...
      <ul>
        ...
        <li><code>option domain-name</code> (opción 15, el dominio de búsqueda DNS que el cliente agrega a los nombres cortos). Validarla exigiría resolver un nombre al cargar la configuración, algo que este demonio nunca hace a propósito, y un dominio de búsqueda muchas veces no tiene registro propio, así que comprobar si resuelve rechazaría los válidos</li>
...
| `subnet ... { pool { ... } }` | Subnet declaration with its dynamic pools. Several `pool { }` blocks are accepted in one subnet, as in `isc-dhcp-server`: each carries its own `range` lines, its own `min`/`default`/`max-lease-time`, and its own access rules (`allow`/`deny members of "<class>"`, `allow`/`deny unknown-clients`, `allow`/`deny known-clients`). A client is served by the first pool that admits it, in the order they are written, so the most specific pool goes first. A pool with no rules admits everyone | Declaración de subred con sus pools dinámicos. Se admiten varios bloques `pool { }` en una misma subred, como en `isc-dhcp-server`: cada uno lleva sus propias líneas `range`, sus propios `min`/`default`/`max-lease-time` y sus propias reglas de acceso (`allow`/`deny members of "<clase>"`, `allow`/`deny unknown-clients`, `allow`/`deny known-clients`). Al cliente lo atiende el primer pool que lo admite, en el orden en que están escritos, así que el pool más específico va primero. Un pool sin reglas admite a todos | Declaración de subred con pool de bloqueo dinámico |
| `host NAME { hardware ethernet MAC; fixed-address IP; }` | Static host reservation. The `fixed-address` is validated at config load (and on `SIGHUP` reload): it must be a syntactically valid IPv4 address, must belong to the configured `subnet`, and must not be that subnet's network or broadcast address. It must also not fall inside `pool { range ... }`, nor equal the `server-identifier`, nor be assigned to more than one host. Any of these rejects the configuration with a clear error instead of being silently accepted. The `hardware ethernet` is validated separately and more leniently: it must be a complete MAC (six colon-separated hex pairs, either case). An entry whose MAC is malformed or incomplete, or that is missing either directive, is skipped with a `WARNING` naming the host, and the daemon still starts — one bad reservation never takes DHCP down for the whole network. A MAC declared in more than one `host` is the exception: it rejects the configuration, the same criterion `tools/pyleases.sh` applies to its ACL files, where a repeated MAC aborts the run before anything is written | Reserva estática de host. La `fixed-address` se valida al cargar la configuración (y al recargar con `SIGHUP`): debe ser una dirección IPv4 sintácticamente válida, debe pertenecer a la `subnet` configurada y no puede ser la dirección de red ni la de broadcast de esa subred. Tampoco puede caer dentro de `pool { range ... }`, ni ser igual al `server-identifier`, ni estar asignada a más de un host. Cualquiera de estos casos rechaza la configuración con un error claro, en vez de aceptarse en silencio. La `hardware ethernet` se valida aparte y con menos rigor: debe ser una MAC completa (seis pares hexadecimales separados por dos puntos, en cualquier caja). Una entrada con la MAC malformada o incompleta, o a la que le falte alguna de las dos directivas, se descarta con un `WARNING` que nombra el host, y el demonio arranca igual — una reserva mal escrita nunca deja sin DHCP a toda la red. La excepción es una MAC declarada en más de un `host`: eso rechaza la configuración, el mismo criterio que aplica `tools/pyleases.sh` sobre sus archivos ACL, donde una MAC repetida aborta la corrida antes de escribir nada |
| `class "blockdhcp" { ... }` / `subclass "blockdhcp" ...` / `deny members of "blockdhcp";` | MAC-based DHCP block list. Any class name is accepted, not just `blockdhcp`: every `subclass "NAME" 1:<mac>;` line puts that MAC in that class, and a pool then admits or refuses it with its own `allow`/`deny members of "NAME";`. Only `subclass` lines carry membership — the `class "blockdhcp" { match pick-first-value (...); }` block is syntax boilerplate a `subclass` requires to be valid, never parsed or acted on itself. As in `isc-dhcp-server`, what actually enforces the block is `deny members of "blockdhcp";` inside the `pool`: without that line the listed MACs are served normally. The MAC must be a complete one (six colon-separated hex pairs, either case); a malformed or truncated one is skipped with a `WARNING` naming the rejected value instead of being added to the block list, where it would have inflated the count without ever matching a real client | Lista de bloqueo DHCP por MAC. Se admite cualquier nombre de clase, no solo `blockdhcp`: cada línea `subclass "NOMBRE" 1:<mac>;` mete esa MAC en esa clase, y luego un pool la admite o la rechaza con su propio `allow`/`deny members of "NOMBRE";`. Solo las líneas `subclass` definen pertenencia — el bloque `class "blockdhcp" { match pick-first-value (...); }` es sintaxis de relleno que una `subclass` necesita para ser válida, nunca se parsea ni se actúa sobre él. Igual que en `isc-dhcp-server`, lo que aplica el bloqueo es `deny members of "blockdhcp";` dentro del `pool`: sin esa línea, las MAC listadas reciben dirección con normalidad. La MAC debe estar completa (seis pares hexadecimales separados por dos puntos, en cualquier caja); una malformada o truncada se descarta con un `WARNING` que nombra el valor rechazado, en vez de entrar a la lista de bloqueo, donde habría inflado el conteo sin coincidir jamás con un cliente real |
...
| `min-lease-time`, `default-lease-time`, `max-lease-time` (pool level) | Validated together; `default-lease-time` is granted to pool (unknown) clients — never to a blocked one, which gets no lease at all | Se validan en conjunto; `default-lease-time` se entrega a clientes del pool (desconocidos) — nunca a uno bloqueado, que no recibe ningún lease |
| `option routers`, `option broadcast-address`, `option domain-name-servers` | Standard DHCP options. `option routers` accepts a comma-separated list (as in `isc-dhcp-server`), but only the first IP is used — extra entries are ignored with a `WARNING`. That first IP must not fall inside `pool { range ... }`, the same guard `server-identifier` carries: otherwise the daemon could hand a client the LAN's own gateway address | Opciones DHCP estándar. `option routers` acepta una lista separada por comas (como en `isc-dhcp-server`), pero solo se usa la primera IP — las entradas adicionales se ignoran con un `WARNING`. Esa primera IP no puede caer dentro de `pool { range ... }`, la misma guarda que lleva `server-identifier`: de lo contrario el demonio podría entregarle a un cliente la dirección de la propia puerta de enlace de la red |
  ...
  <tr>
    <td style="width: 50%; vertical-align: top;">
      ...
      <ul>
        ...
        <li>File paths: 24 characters, the length of <code>/etc/pydhcp/pydhcpd.conf</code>.</li>
    ...
    <td style="width: 50%; vertical-align: top;">
      ...
      <ul>
        ...
        <li>Rutas de archivo: 24 caracteres, el largo de <code>/etc/pydhcp/pydhcpd.conf</code>.</li>
...
|----------|-----------------|---------|
| Authorized client with static IP (renewal) / Cliente autorizado con IP estática (renovación) | `DHCPREQUEST for 192.168.0.50 (192.168.0.2) from aa:bb:cc:dd:ee:ff via enpXsX`<br>`DHCPACK on 192.168.0.50 to aa:bb:cc:dd:ee:ff (FOO) via enpXsX` | `REQUEST from aa:bb:cc:dd:ee:ff (FOO)`<br>`ACK aa:bb:cc:dd:ee:ff → 192.168.0.50`<br>`(lease 2592000s)` |
| Unknown client entering the block pool / Cliente desconocido ingresando al pool de bloqueo | `DHCPDISCOVER from bb:cc:dd:ee:ff:aa via enpXsX`<br>`DHCPOFFER on 192.168.0.230 to bb:cc:dd:ee:ff:aa (BAR) via enpXsX`<br>`DHCPREQUEST for 192.168.0.230 (192.168.0.2) from bb:cc:dd:ee:ff:aa (BAR) via enpXsX`<br>`DHCPACK on 192.168.0.230 to bb:cc:dd:ee:ff:aa (BAR) via enpXsX` | `DISCOVER from bb:cc:dd:ee:ff:aa (BAR)`<br>`OFFER bb:cc:dd:ee:ff:aa → 192.168.0.230`<br>`REQUEST from bb:cc:dd:ee:ff:aa (BAR)`<br>`ACK bb:cc:dd:ee:ff:aa → 192.168.0.230`<br>`(lease 60s)` |
...
<table>
  ...
  <tr>
    <td style="width: 50%; vertical-align: top;">
      <b>pydhcpd</b> also accepts <code>not authoritative;</code>, the same standard <code>dhcpd.conf</code> syntax, but it is <b>not</b> equivalent to omitting <code>authoritative;</code> — see below.
    </td>
    <td style="width: 50%; vertical-align: top;">
