...
| isc-dhcp-server | pydhcp | Description | Descripción |
|---|---|---|---|
| `/etc/dhcp/dhcpd.conf` | `/etc/pydhcp/pydhcpd.conf` | Same syntax | Misma sintaxis |
| `/var/lib/dhcp/dhcpd.leases` | `/etc/pydhcp/pydhcpd.leases` | Same format | Mismo formato |
...
| `dhcpd -t -cf /etc/dhcp/dhcpd.conf` | `pydhcpd.py -t -cf /etc/pydhcp/pydhcpd.conf` | Config syntax test | Prueba de sintaxis de la configuración |
...
├── README.md
├── acl/
│   └── blockdhcp.txt   # pydhcp's own block list (deployed only when absent)
├── init.d/
│   └── pydhcpd         # init.d wrapper (replaces /etc/init.d/isc-dhcp-server)
└── tools/
    ├── bkstack.sh      # Backup tool for pydhcp and uhm (see Tools section)
...
| `/etc/acl/acl_mac/` (administrator's own ACL lists) | ⛔ preserved | ⛔ preserved |
...
|-------------|-------------|------|
| `/etc/pydhcp/pydhcpd.conf` | Main configuration file | Archivo de configuración principal |
| `/etc/pydhcp/pydhcpd.leases` | Active leases database | Base de datos de concesiones activas |
...
<table>
  <tr>
    <td style="width: 50%; vertical-align: top;">
      ...
      ACL directories: <code>/etc/acl/acl_mac/</code> (administrator's own, authorized: <code>mac-limited.txt</code>, <code>mac-unlimited.txt</code>) and <code>/etc/pydhcp/acl/</code> (pydhcp's own, blocked: <code>blockdhcp.txt</code>).<br>
    ...
    <td style="width: 50%; vertical-align: top;">
      ...
      Directorios ACL: <code>/etc/acl/acl_mac/</code> (propios del administrador, autorizados: <code>mac-limited.txt</code>, <code>mac-unlimited.txt</code>) y <code>/etc/pydhcp/acl/</code> (propio de pydhcp, bloqueados: <code>blockdhcp.txt</code>).<br>
