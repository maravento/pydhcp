...
| Shared config: network values and ACL paths generated by `pyinstall.sh`; timers and WPAD/ping-check flags added by `tools/pyleases.sh` if missing | Configuración compartida: valores de red y rutas ACL generados por `pyinstall.sh`; temporizadores y flags de WPAD/ping-check agregados por `tools/pyleases.sh` si faltan | `/etc/pydhcp/pydhcp.env` |
...
| `deny duplicates;` | Re-offer the MAC's existing lease IP on DISCOVER, instead of allocating a new one | Reofrece la IP del lease existente de la MAC en el DISCOVER, en vez de asignar una nueva |
| `one-lease-per-client true;` | Release the MAC's old lease only after the new one is confirmed (not before) | Libera el lease anterior de la MAC sólo después de confirmar el nuevo (no antes) |
...
> **Config source**: `pyinstall.sh` creates `/etc/pydhcp/pydhcp.env` on a fresh install with the network values collected during setup (DHCP server IP, netmask, block-pool range, DNS servers) plus the ACL path variables (`ACL_PATH`, `ACL_MAC_PATH`, `ACL_DHCP_PATH`, `ACL_MAC_PROXY`, `ACL_MAC_UNLIMITED`, `ACL_BLOCK_FILE`) — the single source of truth for them. `pyinstall.sh` also creates `/etc/acl/acl_mac/` and `/etc/acl/acl_dhcp/` with their three ACL files (`blockdhcp.txt`, `mac-proxy.txt`, `mac-unlimited.txt`), empty and mode 600, regardless of whether `pyleases.sh` is ever used — so these paths are always valid, even for a bare `pydhcp` install. `pyleases.sh` never asks for any of this: it verifies the network keys are present and aborts if `pydhcp.env` or any of them is missing, then adds only its own remaining keys (timers, WPAD/ping-check flags) if they aren't already there.
>
> **Origen de la configuración**: `pyinstall.sh` crea `/etc/pydhcp/pydhcp.env` en una instalación nueva con los valores de red recopilados durante la configuración (IP del servidor DHCP, máscara, rango del pool de bloqueo, servidores DNS) más las variables de rutas ACL (`ACL_PATH`, `ACL_MAC_PATH`, `ACL_DHCP_PATH`, `ACL_MAC_PROXY`, `ACL_MAC_UNLIMITED`, `ACL_BLOCK_FILE`) — la única fuente de verdad para ellos. `pyinstall.sh` también crea `/etc/acl/acl_mac/` y `/etc/acl/acl_dhcp/` con sus tres archivos ACL (`blockdhcp.txt`, `mac-proxy.txt`, `mac-unlimited.txt`), vacíos y en modo 600, sin importar si `pyleases.sh` llega a usarse alguna vez — así estas rutas siempre son válidas, incluso en una instalación de `pydhcp` sin extras. `pyleases.sh` nunca pregunta nada de esto: verifica que las claves de red estén presentes y aborta si falta `pydhcp.env` o alguna de ellas, y luego agrega sólo sus propias claves restantes (temporizadores, flags de WPAD/ping-check) si todavía no están.
  ...
  <tr>
    ...
    <td style="width: 50%; vertical-align: top;">
      <ul>
        ...
        <li><code>pyleases.sh</code> reconstruye completamente <code>/etc/pydhcp/pydhcpd.conf</code> en cada ejecución a partir de sus archivos ACL y <code>pydhcp.env</code>. Cualquier edición manual a <code>pydhcpd.conf</code> — incluyendo lease times, pools o directivas personalizadas — se perderá. Si gestiona <code>pydhcpd.conf</code> manualmente, no utilice <code>pyleases.sh</code>.</li>
      </ul>
    </td>
  </tr>
</table>
