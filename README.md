...
| `LOG_FILE` | *(logged via syslog in isc-dhcp-server)* | Log file path, used only by `init.d/pydhcpd` (`pydhcpd.py` resolves its own log destination before it can read this file) | Ruta del archivo de log, usada solo por `init.d/pydhcpd` (`pydhcpd.py` resuelve su propio destino de log antes de poder leer este archivo) |
